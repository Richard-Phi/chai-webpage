...
class 尺素复合体分析器 extends 复合体分析器<默认部件分析, 默认复合体分析> {
  static readonly type = "尺素复合体";

  // 本地缓存，存放独立分析器生成的结果（未污染）
  private 独立部件分析结果 = new Map<部件, 默认部件分析>();
  ...
  // ★★★ 重写查找方法：优先从独立缓存取 ★★★
  查找部件分析结果(部件: 部件): 默认部件分析 | undefined {
    return this.独立部件分析结果.get(部件);
  }

  查找动态部件分析结果(部件: 部件): 优先表<默认部件分析> | undefined {
    return undefined;
  }
  ...
  分析(复合体: 复合体) {
    // 1. 创建独立的部件分析器（不受污染）
    const 原始部件分析器 = new 默认部件分析器(this.配置);

    // 2. 递归收集所有子部件，并用独立分析器分析
    const 待分析部件列表: 部件[] = [];
    this.收集所有部件(复合体, 待分析部件列表);

    // 3. 分析每个部件，存入本地缓存
    for (const 部件 of 待分析部件列表) {
      const 分析结果 = 原始部件分析器.分析(部件);
      if (分析结果.ok) {
        this.独立部件分析结果.set(部件, 分析结果.value);
      }
    }

    // 4. 调用父类的顺序取根（它会通过 查找部件分析结果 拿到我们独立缓存中的数据）
    let 字根序列 = this.顺序取根(复合体);

    // ★★★ 5. 特殊处理：当且仅当字根序列长度为 1 时，取第一部分的第一根追加 ★★★
    if (字根序列.length === 1) {
      // 获取复合体的第一部分（按笔顺的第一个部分）
      const 第一部分 = this.获取第一部分(复合体);
      if (第一部分) {
        // 从独立缓存中获取第一部分的拆分结果
        const 第一部分分析 = this.独立部件分析结果.get(第一部分);
        if (第一部分分析) {
          const 首根 = 第一部分分析.字根序列[0];
          if (首根) {
            // 追加到唯一的字根后面
            字根序列.push(首根);
          }
  ...
  // ★★★ 辅助：获取复合体的第一部分（按笔顺） ★★★
  private 获取第一部分(复合体: 复合体): 部件 | undefined {
    if (!复合体.笔顺 || 复合体.笔顺.length === 0) return undefined;
    const 第一部分索引 = 复合体.笔顺[0]!.index;
    if (第一部分索引 >= 复合体.部分列表.length) return undefined;
    const 部分 = 复合体.部分列表[第一部分索引];
    if (!部分) return undefined; // ← 添加空值检查
    if (是部件(部分)) {
      return 部分;
    }
    // 如果第一部分本身是复合体，递归获取它的第一个部件子部分
    return this.递归获取第一个部件(部分);
  }

  // ★★★ 辅助：递归获取复合体中的第一个部件 ★★★
  private 递归获取第一个部件(字形: 字形): 部件 | undefined {
    if (是部件(字形)) {
      return 字形;
    }
    // 按笔顺取第一个子部分
    if (字形.笔顺 && 字形.笔顺.length > 0) {
      const 索引 = 字形.笔顺[0]!.index;
      if (索引 < 字形.部分列表.length) {
        const 子部分 = 字形.部分列表[索引];
        if (!子部分) return undefined; // ← 添加空值检查
        return this.递归获取第一个部件(子部分);
      }
    }
    return undefined;
  }
  ...
  // ★★★ 辅助：递归收集所有部件 ★★★
  private 收集所有部件(字形: 字形, 收集列表: 部件[]) {
    if (是部件(字形)) {
      收集列表.push(字形);
    } else {
      for (const 部分 of 字形.部分列表) {
        if (部分) { // 防御性检查
          this.收集所有部件(部分, 收集列表);
        }
      }
