...
class 尺素部件分析器 extends 部件分析器<默认部件分析> {
  static readonly type = "尺素"; // ★★★ 配置里要填这个名称 ★★★
  ...
  分析(部件: 部件) {
    ...
    if (
      最终分析.字根序列.length === 1 &&
      !(最终分析.字根序列[0] instanceof 笔画) &&
      typeof 最终分析.字根序列[0] === "object" &&
      "字符" in 最终分析.字根序列[0]
    ) {
      const 唯一部件 = 最终分析.字根序列[0];
      const 二次分析 = 唯一部件.给出部件分析(this.配置);
      if (二次分析.ok && 二次分析.value.全部拆分方式.length > 1) {
        // 取第二种拆分方案（索引 1）的首字根
        const 有效拆分 = 二次分析.value.全部拆分方式[1];
        if (有效拆分 && 有效拆分.拆分方式.length > 0) {
          // ★★★ 替换字根序列为拆碎版的首字根 ★★★
          最终分析.字根序列 = [最终分析.字根序列[0], 有效拆分.拆分方式[0]!.字根];;
        }
      }
    }else if (
      最终分析.字根序列.length === 1 &&
      ...
      const 笔画字根 = 最终分析.字根序列[0];
      最终分析.字根序列 = [笔画字根, 笔画字根]; // 变成两个
    }
